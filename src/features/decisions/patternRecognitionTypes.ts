// Pattern Recognition Engine — Proactive insights from decision history
// After 3+ decisions, the app pushes personalized pattern insights
    ...
    if (topCategory && topCategory[1] >= 2) {
      ...
      if (avgSat > 0 && avgSat < 3) {
        insights.push({
          ...
          title: `Your ${topCategory[0]} decisions aren't satisfying`,
          description: `You've made ${topCategory[1]} decisions in this area with below-average satisfaction. Consider slowing down and using the full analysis framework.`,
          evidence_summary: `${topCategory[1]} decisions, avg satisfaction ${avgSat.toFixed(1)}/5`,
          evidence_count: topCategory[1],
  ...
  if (decisions.length >= 2) {
    ...
    if (fastDecisions.length >= 2) {
      insights.push({
        ...
        title: 'You rush high-stakes decisions',
        description: `${fastDecisions.length} important decisions were made in under an hour. High-importance choices benefit from deliberate pacing.`,
        evidence_summary: `${fastDecisions.length} rushed high-importance decisions`,
        evidence_count: fastDecisions.length,
  ...
  if (biases.length > 0) {
    ...
    if (topBias && topBias.count >= 2) {
      insights.push({
        ...
        title: `${topBias.bias_name} is a recurring pattern`,
        description: `This bias has appeared in ${topBias.count} of your decisions. Awareness is the first step — try actively counter-arguing before deciding.`,
        evidence_summary: `Detected ${topBias.count} times across your decisions`,
        evidence_count: topBias.count,
