// RegretRiskBadge — Visual indicator for regret risk level
...
export function RegretRiskBadge({ score, size = 'medium' }: RegretRiskBadgeProps): JSX.Element {
  const getLevel = (s: number): { label: string; color: string } => {
    if (s <= 30) return { label: 'Low Risk', color: colors.status.success };
    if (s <= 60) return { label: 'Moderate', color: colors.status.warning };
    return { label: 'High Risk', color: colors.status.error };
  };
